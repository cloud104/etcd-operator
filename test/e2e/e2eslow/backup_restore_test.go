...
func getEndpoints(kubeClient kubernetes.Interface, secureClient bool, namespace, clusterName string) ([]string, error) {
	podList, err := kubeClient.CoreV1().Pods(namespace).List(context.Background(), k8sutil.ClusterListOpt(clusterName))
	if err != nil {
		return nil, err
	}
...
func testEtcdBackupOperatorForS3Backup(t *testing.T, clusterName, operatorClientTLSSecret, s3Path string) {
	...
	eb, err := f.CRClient.EtcdV1beta2().EtcdBackups(f.Namespace).Create(context.Background(), backupCR, metav1.CreateOptions{})
	if err != nil {
		t.Fatalf("failed to create etcd backup cr: %v", err)
	}
	defer func() {
		if err := f.CRClient.EtcdV1beta2().EtcdBackups(f.Namespace).Delete(context.Background(), eb.Name, metav1.DeleteOptions{}); err != nil {
	...
	err = retryutil.Retry(time.Second, 4, func() (bool, error) {
		reb, err := f.CRClient.EtcdV1beta2().EtcdBackups(f.Namespace).Get(context.Background(), eb.Name, metav1.GetOptions{})
		if err != nil {
...
func testEtcdRestoreOperatorForS3Source(t *testing.T, clusterName, s3Path string) {
	...
	er, err := f.CRClient.EtcdV1beta2().EtcdRestores(f.Namespace).Create(context.Background(), er, metav1.CreateOptions{})
	if err != nil {
		t.Fatalf("failed to create etcd restore cr: %v", err)
	}
	defer func() {
		if err := f.CRClient.EtcdV1beta2().EtcdRestores(f.Namespace).Delete(context.Background(), er.Name, metav1.DeleteOptions{}); err != nil {
			t.Fatalf("failed to delete etcd restore cr: %v", err)
		}
	}()

	err = retryutil.Retry(10*time.Second, 1, func() (bool, error) {
		er, err := f.CRClient.EtcdV1beta2().EtcdRestores(f.Namespace).Get(context.Background(), er.Name, metav1.GetOptions{})
		if err != nil {
