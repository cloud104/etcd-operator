...
func (r *Restore) prepareSeed(er *api.EtcdRestore) (err error) {
	...
	if err != nil {
		r.logger.Warningf("Found a error: %s", err.Error())
	}
	...
	err = retryutil.Retry(2, 1, func() (bool, error) {
		ec, err = r.etcdCRCli.EtcdV1beta2().EtcdClusters(r.namespace).Get(context.Background(), clusterName, metav1.GetOptions{})
		if err != nil {
			return false, err
		}
		ec.Spec.Paused = false
		_, err = r.etcdCRCli.EtcdV1beta2().EtcdClusters(r.namespace).Update(context.Background(), ec, metav1.UpdateOptions{})
		if err != nil {
