...
func (m *Monkeys) CrushPods(ctx context.Context, c *CrashConfig) {
	...
	for {
		...
		pods, err := m.kubecli.CoreV1().Pods(ns).List(context.Background(), metav1.ListOptions{LabelSelector: ls})
		if err != nil {
		...
		for tokill := range tokills {
			...
			err = m.kubecli.CoreV1().Pods(ns).Delete(context.Background(), tokill, *metav1.NewDeleteOptions(0))
			if err != nil {
